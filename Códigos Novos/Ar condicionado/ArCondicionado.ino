...
// Configurações MQTT
String publish_topic = "LMM-ArCondicionado";      //Tópico para envio dos dados obtidos
...
unsigned long irCooldown = 12000; // 10 segundos de "delay" entre acionamentos
int lastPresenca = 0;
bool irDisparado = false;
...
void setup() {
  ...
  // Configurar MQTT
  client.setServer("aefef4e8d2044fcebe74d885d8681523.s1.eu.hivemq.cloud", 8883); //mudou aqui
...
void loop() {
  ...
  if (currentMillis - previousLoopMillis >= loopInterval) {
    ...
    // ------------------------------
    // Detecta borda de subida do PIR
    if (presenca == 1 && lastPresenca == 0 && !irDisparado) {
      Serial.println("PIR ativado - enviando sinal IR para desligar ar.");
      irsend.sendRaw(desligarAr, sizeof(desligarAr) / sizeof(desligarAr[0]), 38);
      irDisparado = true;
      previousIRMillis = currentMillis;
    }

    // Libera novo disparo após o tempo de cooldown
    if (irDisparado && (currentMillis - previousIRMillis >= irCooldown)) {
      irDisparado = false;
      Serial.println("Sinal IR pronto para novo disparo.");
    }
  ...
  if (currentMillis - previousSendMillis >= sendInterval) {
    ...
    if (client.connected()) {
      ...
      client.publish("sensor/presenca",String(irDisparado ? "1" : "0").c_str());
    } else {
